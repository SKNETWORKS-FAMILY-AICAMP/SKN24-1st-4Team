...
| 이름 | 역할 | GitHub |
|---|---|---|
| 김규호 | DB 생성 및 적재 파이프라인 구축 | https://github.com/kyu5KIm |
| 김지원 | FAQ 데이터 크롤링 및 테이블 설계 | https://github.com/edu-ai-jiwon |
| 류지우 | Streamlit 개발 및 DB 연동 | https://github.com/jia11234 |
| 박수영 | DB 생성 및 FAQ 크롤링 | https://github.com/suyoung6279 |
...
## 2-1. 프로젝트 명
**Seoul Way**  
서울시 자치구별 차량 등록 밀도와 교통혼잡(월별) 퍼센티지 통계 사이트

## 2-2. 배경

![image](./src/YearlyRoadLength.png.png)
![image](./src/Bar.png.png)
![image](./src/grap.png.jpg)

- 참고 기사  
  https://news.nate.com/view/20250728n04175  
- 데이터 출처  
...
- 도로종류별 기준속도를 적용해 혼잡 퍼센티지(속도저하율)를 산출하고, 자치구 내 비교가 가능하도록 표준화한다.
...
| ID | 요구사항 | 구현 여부 | 비고 |
|---|---|---|---|
| R1 | 교통 DB 생성 및 테이블 구축 | 완료 | create database traffic테이블 삭제 → 재생성. - 스키마 구조 확정 |
| R2 | 도로 정보 저장 (tbl_road) | 완료 | tbl_road 생성 : road_id(PK), 도로명, 시점/종점, 방향 설계 |
| R3 | 제한속도 테이블 및 FK | 완료 | tbl_speed_limit : road_code  → tbl_road(road_id) FK 설정 |
| R4 | 혼잡도 테이블 | 완료 | tbl_congestion :(road_code, date_id) 복합 PK  평균속도 컬럼 설계 |
| R5 | 차량 등록 현황 | 완료 | bl_registed_car : (registed_region, registed_month) 복합 PK |
| R6 | FAQ 크롤링 | 완료 | tbl_faq : 키워드: 혼잡 / 교통 : 총 27건 크롤링 |

---

# 6. ERD

![image](./src/erd.png.png)

## 6-1. DA 모델

본 프로젝트의 ERD는 도로(링크) 엔터티를 중심으로 제한속도/폰잡도가 연결되며, 자치구(권역구분) 기반 차량 등록 현황과 FAQ 테이블을 별도로 구성한다. 
...
- 김지원: FAQ 데이터를 크롤링하는 과정에서 세부 주제에 적합한 키워드 선정과 HTML 구조를 파악하는 데 어려움을 겪었다. 특히 원하는 요소가 selector로 정상적으로 선택되지 않아 문제 해결에 시간이 소요되었으나, 팀원에게 질문하며 HTML 태그 구조와 BeautifulSoup selector 문법의 차이를 이해하게 되었다. 이 과정에서 반복문을 활용한 텍스트 추출 방식에 대한 이해를 점진적으로 높일 수 있었다.
