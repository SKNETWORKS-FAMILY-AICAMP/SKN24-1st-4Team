...
| --- | --- | --- |
| 김규호 | DB생성 및 적재
파이프라인 구축 | https://github.com/kyu5KIm |
| 김지원 | FAQ 데이터 크롤링 및
테이블 설계 | https://github.com/edu-ai-jiwon |
| 류지우 | streamlit 개발 및 DB연동  | https://github.com/jia11234 |
| 박수영 | DB생성 및 FAQ 크롤링 | https://github.com/suyoung6279 |
...
![image.png](attachment:07e75cca-c6f3-475a-9e6e-564b206c08c4:image.png)

![rMateChartH5.jpg](attachment:ef1cc74e-190f-4199-959d-9d34d5e2bf2a:rMateChartH5.jpg)

![image.png](attachment:7890d923-9519-4269-b2a0-3c0eb00d4979:image.png)

[https://news.nate.com/view/20250728n04175](https://news.nate.com/view/20250728n04175?utm_source=chatgpt.com)

https://data.seoul.go.kr/dataList/10735/S/2/datasetView.do

서울시 「2025년도 도로 통계」에 따르면 서울의 도로 총연장은 최근 몇 년간 전반적으로 증가해 2024년 기준 8,343km 수준으로 집계됩니다. 도로가 늘어 실제로 혼잡이 줄어들었는지 확인해 보기 위해 저희는 서울 내 승용차 통행속도를 보았습니다.
...
추가로 서울시설공단 FAQ 중 ‘혼잡/교통’ 관련 문항을 크롤링하여(총 27개) DB에 적재함으로써 사용자 정보 탐색 기능을 보완하였다.
...
- 교통 혼잡을 제대로 비교하려면 평균속도, 도로 종류/제한속도, 도로 길이, 자치구별 차량 등록 대수가 함께 필요하지만, 데이터가 서로 다른 형태로 분산되어 있어 한 번에 분석하기 어렵다.
...
- 서울시 자치구별 차량 밀도, 주요 도로 월평균 속도 데이터를 통합해 한 화면에서 보여준다.
- 도로 종류별 기준 속도를 적용해 혼잡 퍼센티지(속도 저하율)를 산출하고, 자치구 간 비교가 가능하도록 표준화한다.
- 월별 통계를 제공해 자치구별 혼잡의 변화 추세를 확인할 수 있게 한다.
- 1km당 등록 차량 밀도(차량/도로 km)를 함께 계산해, 도로 공급 대비 차량 수요가 높은 혼잡 취약 자치구를 식별한다.
- 결과 지표를 정책·운영·서비스 기획(우선순위 설정, 지역 특성 분석 등)에 활용할 수 있는 형태로 제공한다.
- 교통/혼잡 관련 FAQ 조회 기능을 제공해 사용자의 정보 탐색을 돕는다.

## 2-7. **주요 타겟 사용자**

- 서울 생활권(거주·방문·통근) 의사결정을 하는 일반 이용자 및 사회 초년생
- 서울시 및 자치구의 교통 운영·민원 대응·투자 담당자
- 모빌리티/렌터카/물류 기업의 전략 담당자

# 3. 기술 스택

- **Python** (pandas, BeautifulSoup, mysql-connector)
- MySQL
- Stremlit
- Figma
- git hub / notion
...
![image.png](attachment:2606f7bd-a0d8-42c1-a0c8-1fd29a35bc0a:image.png)

# 5. 요구사항명세서

| ID | 요구사항 | 구현 여부 | 비고 |
| --- | --- | --- | --- |
| R1 | 교통 DB(traffic) 생성 및 테이블 구축 | 완료 | -`create database traffic`테이블 삭제 → 재생성. - 스키마 구조 확정 |
| R2 | 도로(링크) 정보 저장 (tbl_road) | 완료 | -`tbl_road` 생성
- road_id(PK), 도로명, 시점/종점, 방향 설계 |
| R3 | 제한속도 저장(tbl_speed_limit) 및 도로 FK 연결 |  완료 | -`tbl_speed_limi`
- `road_code`  → `tbl_road(road_id)` FK 설정 |
| R4 | 혼잡도(평균속도) 저장 (tbl_congetion) 및 도로 FK 연결 | 완료 | -`tbl_congestion` 
-`(road_code, date_id)` 복합 PK
- 평균속도 컬럼 설계 |
| R5 | 구별 차량 등록 현황 저장 (tbl_registed_car) | 완료 | - `bl_registed_car` 
-`(registed_region, registed_month)` 복합 PK |
| R6 | FAQ 크롤링 후 DB 적재(tbl_faq) | 완료 | - 키워드: 혼잡 / 교통
- 총 27건 크롤링
-`tbl_faq`에 INSERT |

# 6. ERD

![image.png](attachment:a7f5dd2f-9dee-4b4e-9e89-4c5c2cf1390b:image.png)

## 6-1. DA 모델(ERD)

본 프로젝트의 ERD는 도로(링크) 엔터티를 중심으로 제한속도/혼잡도가 연결되며, 자치구(권역구분) 기반 차량 등록 현황과 FAQ 테이블을 별도로 구성한다. 

- 도로(tbl_roda): roda_id(PK), registed_region, 도로명/시점/종점/방향
- 제한속도(tbl_speed_limit): road_conde(FK → tbl_road.road_id), 제한속도
- 혼잡도(tbl_congestion): road_code(FK), data_id, average_speed(PK: road_code+date_id)
- 구별차량현황(tbl_registed_car): registed_region+registed_month( PK)
- FAQ(tbl_faq): faq_id(PK), faq_title, faq_contents, faq_section
...
- 처리 내용: 월별(24개월) · 지역별(25개 구) 차량 등록대수 파싱
- 전처리 사항: 총계() 값 제거 및 결측치 처리
- 최종 적재 개수: 총 600건 (24개월 × 25개 지역)
- 적재 테이블: `tbl_registed_car`

### 7-1-3. 도로 기본 정보 DB 적재 결과

- 데이터 출처: 서울시 월별 차량 통행속도 데이터(`서울시 차량통행속도.xlsx`)
...
※ 차량 등록 대수는 혼잡 퍼센티지 산출에는 직접 사용하지 않고, 보조 지표 계산에 활용한다.

1. **차량 정규화 혼잡 지수 산출(보조 지표)**
...
1. **결측/오류값 처리 정책**
- 본 데이터에서는 월평균 속도 0값이 관측되지 않아, 0값(결측/오류) 처리 규칙은 분석 범위에서 제외하였다.
- 차량 등록 밀도 산출 시 도로 길이가 0으로 산성되는 경우는 분석 대상에서 제외한다.
1. **혼잡 퍼센티지 해석 가이드(→ 교통 정체 기준을 바탕으로 산정)**

![traffic_level.png](attachment:dc260819-1925-41a5-aee0-688fbaefa4d3:traffic_level.png)
...
- 김규호: 월별 구조가 다른 대용량 교통 데이터를 엑셀에서 DB로 적재하며, 작은 오류로 전체 작업이 멈추는 경험을 통해 정확한 코드 작성의 중요성과 재실행 가능한 데이터 파이프라인의 필요성을 깨달았다.
